...
class SingleAnalyzerCmaq:
    def __init__(self, model_name, whole_coord, input_data: pd.DataFrame, label_data: pd.Series, pred_data: pd.Series, train_test_id):
        self.model_name = model_name
        self.whole_coord = whole_coord
        self.input_data = input_data
        self.label_data = label_data
    ...
    def _compute_cluster_info(self):
        cluster_info = {}
        for cluster_id in self.train_test_id.keys():
            cluster_test_cmaq = self.train_test_id[cluster_id]['test_cluster']
            cluster_label = self.label_data[np.isin(self.input_data['cmaq_id'], cluster_test_cmaq)]
            cluster_pred = self.pred_data[np.isin(self.input_data['cmaq_id'], cluster_test_cmaq)]
            cluster_r2 = round(r2_score(cluster_label, cluster_pred), 4)
            cluster_mae = round(mean_absolute_error(cluster_label, cluster_pred), 4)
            cluster_info[cluster_id] = {
                'r2':cluster_r2, 'mae':cluster_mae
    ...
    def plot_dataset_map(self):
        for cluster_id in self.train_test_id.keys():
            test_cmaq = self.train_test_id[cluster_id]['test_cluster']
            source_cmaq = self.train_test_id[cluster_id]['train_out_cluster']
            train_cmaq = self.train_test_id[cluster_id]['train_in_cluster']
            test_xy = self.whole_coord.loc[np.isin(self.whole_coord['cmaq_id'], test_cmaq)]
            source_xy = self.whole_coord.loc[np.isin(self.whole_coord['cmaq_id'], source_cmaq)]
            train_xy = self.whole_coord.loc[np.isin(self.whole_coord['cmaq_id'], train_cmaq)]
            plt.scatter(test_xy["cmaq_x"], test_xy["cmaq_y"], s=3, c='red', label="test")
            plt.scatter(source_xy["cmaq_x"], source_xy["cmaq_y"], s=3, c='blue', label="source")
            plt.scatter(train_xy["cmaq_x"], train_xy["cmaq_y"], s=3, c='green', label="train")
            plt.title(f"set{cluster_id}")
            plt.legend()
            plt.show()
